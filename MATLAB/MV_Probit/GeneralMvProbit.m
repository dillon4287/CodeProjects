function [storeSigma0, storeBeta] = GeneralMvProbit(yt, X, Sigma0, b0, B0, tau0, T0, s0, S0,...
    Sims,bn, realzt)
[K,T]=size(yt);
[~,P]=size(X);
KT =K*T;
...
for s = 1:Sims
    ...
    mean(zt,2)';
    % Sample beta
    beta = mvp_betadraw(realzt,surX, Sigma0, subsetIndices, kronB0inv, kronB0invb0);
    mut = reshape(surX*beta,K,T);
    demuyt = zt-mut;
