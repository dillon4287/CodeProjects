...
squaresSampled= 9;
ploton= 0;
K = SeriesPerY*squaresSampled;
Groups{1} = [1,K];
% Groups{2} = [(1:SeriesPerY:K)', (SeriesPerY:SeriesPerY:K)'];
% GroupingInfo = [1:K;
%     (1:SeriesPerY:K)', (SeriesPerY:SeriesPerY:K)']
% CorrType:
...
DataCell = SpatialMLFdataWithMean(SeriesPerY, gridX,...
    squaresSampled, Groups, ploton, T, euclidParama, CorrType);

yt = DataCell{1,1};
Xt = DataCell{1,2};
InfoCell = DataCell{1,3};
SpatialInfo =  DataCell{1,4};
LocationCorrelation = DataCell{1,5};
Factor = DataCell{1,6};
TrueG = DataCell{1,7};
if CorrType == 1
...
factorIden = MakeObsModelIdentity(InfoCell);
StateObsModel = makeStateObsModel(initobsmodel, factorIden,0);
...
[sumFt, sumFt2, sumOM, sumOM2, sumST, sumST2,...
    ...
    SMDFVAR_SimVersion(yt, InfoCell, SpatialInfo, CorrType,...
...
%     sumFactorVar, sumFactorVar2,sumVarianceDecomp,...
%     sumVarianceDecomp2, parama] = ...
%     Smdfavar(yt, Xt, InfoCell, CorrType,...
%     LocationCorrelation, cutpoints, Sims, burnin,ReducedRuns,...
