...
class MultilevelData:
    def __init__(self, T=100, K=9, betas=[1, .5, .5],
                 InfoDict={"L1": [(0, 8)], "L2": [(0, 5), (6, 8)]},
                 gammas=np.array([.2, .2]), factorVariance=np.array([1., 1., 1.]),
                 A=np.ones((9, 3))):
        self.T = T
        self.K = K
        ...
        self.factorVariance = factorVariance
        if self.factorVariance.shape[0] != self.nFactors:
            raise nFactorError
        self.FactorPrecision = reg.Precision(self.gammas, self.factorVariance,
                                             reg.InitializeArVariance(self.gammas, self.factorVariance),
                                             self.T)
        ...
        self.om_variance = np.ones((self.K,))
        self.b0 = np.zeros((self.Betas.shape[0],))
        self.B0 = 10.*np.eye(self.Betas.shape[0])
